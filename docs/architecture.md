...
- **Embedded Assets:** All Typst template files, modular block definitions, and
  ...
  `include_dir!` and `include_str!`. The user only needs a single binary executable.
- **Template Registry & Ejection:** Embedded templates live under
  ...
  discovered by the engine via `include_dir!`. Users can eject the full source tree via
