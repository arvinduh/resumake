...
| **Typst Compiler (Optional)** (only if working on raw Typst templates outside resumake; Resumake uses an embedded in-process Typst engine and does not require an external `typst` CLI to build, test, or run) | `winget install Typst.Typst`                                                       | `brew install typst`                                                                    | `cargo install --locked typst-cli`                                                      |
