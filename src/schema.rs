...
/// GitHub Release asset URL for the JSON Schema matching this build.
///
/// The schema is not committed to the repository (see
/// `.gitignore`) — it is generated fresh by `resumake schema --export`
/// and published as a release asset by `.github/workflows/release.yml`
/// whenever a `vX.Y.Z` tag is pushed. Scaffolds generated by a build that
/// hasn't been released under this exact version yet will point at a URL
/// that 404s; YAML language servers treat that as "no live schema" rather
/// than a hard error, so `content.yaml` remains fully usable either way.
fn schema_url() -> String {
  format!(
    "https://github.com/arvinduh/resumake/releases/download/v{}/resume.schema.json",
...
"#,
  ...
  fn test_generate_init_template() {
    ...
    // version-pinned release-asset URL, not left dangling.
    assert!(!tmpl.contains("RESUMAKE_SCHEMA_URL"));
    assert!(tmpl.contains("/releases/download/v"));
